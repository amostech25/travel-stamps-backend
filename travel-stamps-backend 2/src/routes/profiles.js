...
// Shapes a full user record (with nested relations) into the JSON contract
// the frontend expects, and folds in like counts + "did the current viewer
// like this" without exposing who else liked it.
function shapeProfile(user, viewerId) {
  ...
  return {
    ...
    isFollowedByViewer: viewerId ? user.followers.some((f) => f.followerId === viewerId) : false,
    bio: user.bio
      ? {
    ...
    countries: user.countries.map((c) => ({
      ...
      cities: c.cities.map((city) => ({
        ...
        spots: city.spots.map((s) => ({
          ...
          likedByViewer: viewerId ? s.likes.some((l) => l.userId === viewerId) : false,
        })),
      })),
    })),
  };
...
const fullInclude = {
  ...
  countries: {
    include: {
      cities: {
        include: {
          spots: { include: { references: true, likes: true } },
        },
      },
    },
  },
