...
router.post("/signup", authLimiter, asyncHandler(async (req, res) => {
  ...
  // This is a single-owner personal guide, not a public multi-user
  // platform — only the very first account is ever allowed to be created.
  const existingOwner = await prisma.user.count();
  if (existingOwner > 0) {
    return res.status(403).json({ error: "This guide already has an owner account." });
  }
